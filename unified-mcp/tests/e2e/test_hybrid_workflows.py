...
class TestMarkCommunityAsReadWithHistoryE2E:
    ...
    def test_mark_community_as_read_with_history_completes_end_to_end_with_500_messages(
        ...
        with (
            patch("main.backend_status") as mock_status,
            ...
            mock_history.return_value = {"success": True, "status": {"messages_synced": 500}}
            ...
            mock_sync.return_value = {"success": True, "messages_added": 500, "messages_skipped": 0}
            ...
            result = mark_community_as_read_with_history(
                community_jid=e2e_test_community["community_jid"], sync_timeout=300
            )
    ...
    def test_mark_community_as_read_with_history_handles_baileys_history_sync_failure(
        ...
        assert any(
            step["step"] == "history_sync" and "❌" in step["status"] for step in result["steps"]
        )
    ...
    def test_mark_community_as_read_with_history_handles_go_mark_as_read_failure_after_sync(
        ...
        with (
            ...
            mock_history.return_value = {"success": True, "status": {"messages_synced": 100}}

            # Database sync succeeds
            mock_sync.return_value = {"success": True, "messages_added": 100, "messages_skipped": 0}
        ...
        assert any(
            step["step"] == "database_sync" and "✅" in step["status"] for step in result["steps"]
        )
        assert any(
            step["step"] == "mark_as_read" and "❌" in step["status"] for step in result["steps"]
        )

    def test_mark_community_as_read_with_history_reports_accurate_metrics(self, e2e_test_community):
        ...
        with (
            ...
            # Setup mocks
            mock_status.return_value = {"overall_status": "healthy"}
            mock_history.return_value = {"success": True, "status": {"messages_synced": 250}}
    ...
    def test_mark_community_as_read_with_history_updates_checkpoints_correctly(
        ...
        with (
            ...
            def sync_with_checkpoint():
                ...
                return {"success": True, "messages_added": 100, "messages_skipped": 0}
        ...
        cursor = integration_database.cursor()
        for group_jid in e2e_test_community["group_jids"]:
            cursor.execute("SELECT * FROM sync_checkpoints WHERE chat_jid = ?", (group_jid,))
