...
def list_communities(query: str | None = None, limit: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        ...
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/communities", params=params, timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
...
def query_messages(
    ...
    try:
        ...
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/messages", params=params, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
...
def get_message_stats() -> dict[str, Any]:
    ...
    try:
        response = requests.get(f"{GO_BRIDGE_URL}/api/stats", timeout=SHORT_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except Exception as e:
        print(f"Error getting message stats: {e}")
        return {"success": False, "total_messages": 0, "total_chats": 0, "total_contacts": 0}
...
def send_media_message(
    ...
    try:
        payload = {"chat_jid": chat_jid, "media_path": media_path, "media_type": media_type}
        if caption:
            payload["caption"] = caption

        response = requests.post(
            f"{GO_BRIDGE_URL}/api/messages/send-media", json=payload, timeout=MEDIA_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def delete_message(message_id: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.delete(
            f"{GO_BRIDGE_URL}/api/messages/{message_id}/revoke", timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def list_chats(limit: int = 20, archived: bool = False) -> list[dict[str, Any]]:
    ...
    try:
        ...
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/chats/list", params=params, timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
...
def get_chat_metadata(chat_jid: str) -> dict[str, Any]:
    ...
    try:
        response = requests.get(f"{GO_BRIDGE_URL}/api/chats/{chat_jid}", timeout=SHORT_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def archive_chat(chat_jid: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.post(
            f"{GO_BRIDGE_URL}/api/chats/{chat_jid}/archive", timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def pin_chat(chat_jid: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.post(
            f"{GO_BRIDGE_URL}/api/chats/{chat_jid}/pin", timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def unmute_chat(chat_jid: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.delete(
            f"{GO_BRIDGE_URL}/api/chats/{chat_jid}/mute", timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def search_contacts_v2(query: str) -> list[dict[str, Any]]:
    ...
    try:
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/contacts/search", params={"query": query}, timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
...
def get_contact_details(jid: str) -> dict[str, Any]:
    ...
    try:
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/contacts/details", params={"jid": jid}, timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def get_contact_status(jid: str) -> dict[str, Any]:
    ...
    try:
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/contacts/status", params={"jid": jid}, timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def get_linked_devices() -> dict[str, Any]:
    ...
    try:
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/contacts/linked-devices", timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def block_contact(jid: str | None = None, phone: str | None = None) -> tuple[bool, str]:
    ...
    try:
        ...
        response = requests.post(
            f"{GO_BRIDGE_URL}/api/privacy/block", json=payload, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def unblock_contact(jid: str | None = None, phone: str | None = None) -> tuple[bool, str]:
    ...
    try:
        ...
        response = requests.post(
            f"{GO_BRIDGE_URL}/api/privacy/unblock", json=payload, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def get_blocked_contacts() -> dict[str, Any]:
    ...
    try:
        response = requests.get(f"{GO_BRIDGE_URL}/api/privacy/blocked", timeout=SHORT_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def get_privacy_settings() -> dict[str, Any]:
    ...
    try:
        response = requests.get(f"{GO_BRIDGE_URL}/api/privacy/settings", timeout=SHORT_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def update_last_seen_privacy(value: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.put(
            f"{GO_BRIDGE_URL}/api/privacy/last-seen", json={"value": value}, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def update_status_privacy(value: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.put(
            f"{GO_BRIDGE_URL}/api/privacy/status", json={"value": value}, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def update_online_privacy(value: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.put(
            f"{GO_BRIDGE_URL}/api/privacy/online", json={"value": value}, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def get_business_profile(jid: str) -> dict[str, Any]:
    ...
    try:
        response = requests.get(
            f"{GO_BRIDGE_URL}/api/business/{jid}/profile", timeout=SHORT_TIMEOUT
        )
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def subscribe_to_newsletter(jid: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.post(
            f"{GO_BRIDGE_URL}/api/newsletters/{jid}/subscribe", timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        data = response.json()
        return data.get("success", False), data.get("message", "")
...
def create_newsletter(name: str, description: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(
            f"{GO_BRIDGE_URL}/api/newsletters/create", json=payload, timeout=DEFAULT_TIMEOUT
        )
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def get_newsletter_metadata(jid: str) -> dict[str, Any]:
    ...
    """
    try:
        response = requests.get(f"{GO_BRIDGE_URL}/api/newsletters/{jid}", timeout=SHORT_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except Exception as e:
