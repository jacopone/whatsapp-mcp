...
export class DatabaseService {
    ...
    storeChat(chat) {
      ...
      VALUES (?, ?, ?)
    `);
        stmt.run(chat.jid, chat.name || null, chat.last_message_time?.toISOString() || null);
    }
    /**
     * Store multiple chats in a transaction
     */
    storeChatsInBatch(chats) {
    ...
    `);
        const transaction = this.db.transaction((chats) => {
            for (const chat of chats) {
                insert.run(chat.jid, chat.name || null, chat.last_message_time?.toISOString() || null);
            }
        });
        transaction(chats);
    }
    ...
    storeMessage(msg) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(msg.id, msg.chat_jid, msg.sender || null, msg.content || null, msg.timestamp.toISOString(), msg.is_from_me ? 1 : 0);
    }
    /**
     * Store multiple messages in a transaction (much faster for bulk inserts)
     */
    storeMessagesInBatch(messages) {
        const insert = this.db.prepare(`
      INSERT OR REPLACE INTO messages (id, chat_jid, sender, content, timestamp, is_from_me)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        const transaction = this.db.transaction((messages) => {
            for (const msg of messages) {
                insert.run(msg.id, msg.chat_jid, msg.sender || null, msg.content || null, msg.timestamp.toISOString(), msg.is_from_me ? 1 : 0);
            }
        });
     ...
     */
    getMessagesByChatJID(chatJID, limit = 100) {
        const stmt = this.db.prepare(`
