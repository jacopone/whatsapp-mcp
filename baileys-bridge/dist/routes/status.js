 ...
 */
export function registerStatusRoutes(router, getSock) {
    // POST /status/post - Post a status update
    router.post('/status/post', async (req, res) => {
    ...
    router.post('/status/:status_id/view', async (req, res) => {
    ...
    router.get('/status/privacy', async (req, res) => {
        const sock = getSock();
        if (!sock) {
            return res.status(503).json({
                success: false,
